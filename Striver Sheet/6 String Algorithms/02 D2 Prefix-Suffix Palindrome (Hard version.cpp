...
void sol()
{
    ...
    // remaining
    string rem = str.substr(same, n - 2 * same);
    string ans = str.substr(0, same);

    if (rem.size())
    {
        int pre_palindrome = largest_pre_pal(rem);
        reverse(all(rem));
        int suf_palindrome = largest_pre_pal(rem);
        if (pre_palindrome > suf_palindrome)
        {
            reverse(rem.begin(), rem.end());
            ans += rem.substr(0, pre_palindrome);
        }
        else
        {
            ans += rem.substr(0, suf_palindrome);
        }
    }
