...
struct Node
{
    ...
    set<ull> in_this_tree;
};

void insert(Node *node, ull hash, int pos, string &s, vector<ull> &st, map<char, int> &ord)
{
    if (pos == s.size())
        return;
...
bool check(Node *node, ull hash, int pos, string &s, vector<ull> &st, map<char, int> &ord)
{
    if (pos == s.size() - 1)
        return false;

    for (auto it : node->childrens)
        if (it.second != NULL && it.first != s[pos + 1])
        {
            if (pos < s.size() - 2)
            {
                Node *tmp = it.second->childrens[s[pos + 2]];
                ull new_hash = hash - ord[s[pos]] * st[s.size() - pos - 1] - ord[s[pos + 1]] * st[s.size() - (pos + 1) - 1];
