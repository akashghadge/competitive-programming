...
void help(ll x_origin, ll y_origin, ll radius, unordered_map<string, ll> &st)
{
    for (ll x = x_origin - radius; x <= x_origin + radius; ++x)
    {
        for (ll y = y_origin - radius; y <= y_origin + radius; ++y)
        {
            if (sqrt(pow(x - x_origin, 2) + pow(y - y_origin, 2)) <= radius)
            {
                st[(to_string(x) + ":" + to_string(y))]++;
            }
        }
    }
}
