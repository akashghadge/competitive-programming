...
ll rec(vector<ll> &arr, ll n, ll id, vector<int> curr)
{
    if (id == n)
        return INT_MAX;
    if (curr.size() == 3)
    {
        return 3 * abs((accumulate(all(curr), 0) / 3) - curr[1]);
    }
    ll not_pick = rec(arr, n, id + 1, curr);
    curr.push_back(arr[id]);
    ll pick = rec(arr, n, id + 1, curr);
