...
vector<int> solution(vector<int> &stations, vector<int> &persons)
{
    vector<int> timing(stations.size(), INT_MAX);
    timing[0] = 0;
    for (int i = 0; i < stations.size(); i++)
    {
        ...
        {
            int time = 0;
            for (int j = i; j < timing.size(); j++)
            {
                timing[j] = min(time, timing[j]);
                time++;
            }
        }
        else if (stations[i] == 2)
        {
            int time = 0;
            for (int j = i; j >= 0; j--)
            {
    ...
    {
        int temp = persons[i];
        if (timing[temp - 1] == INT_MAX)
        {
            persons[i] = -1;
            continue;
        }
        persons[i] = timing[temp - 1];
    }
    return persons;
}
int main()
{
    ...
    for (int k = 0; k < T; k++)
    {
        ...
        persons = solution(stations, persons);
