...
void pre_processing()
{
    // for euler phi function
    iota(phi.begin(), phi.end(), 0);

    // just like we are trying for every possible factor from 1 to n
    for (int i = 2; i <= size; i++)
    {
        // if this is prime factor
        if (phi[i] == i)
        {
            // for prime answer is p-1
            phi[i] = i - 1;
