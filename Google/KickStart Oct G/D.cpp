...
ll solve(vi &curr, vvi &arr, int n, int k)
{
    ll mx_sum = 0;
    ll sum = 0;
    bool side = 1;
    ...
    for (auto pt : curr)
    {
        ll new_ground = arr[pt][0];
        ll new_alt = arr[pt][1];
        ll rect = arr[pt][2];
        if (new_alt > alt)
            continue;
        if (side and new_ground - ground >= 0)
        {
            sum += rect;
        }
        else if (!side and ground - new_ground >= 0)
        {
            sum += rect;
        }
        else
...
void sol()
{
    ...
    do
    {
        mx = max(solve(curr, arr, n, k), mx);
        // prn(mx);
        // cout << "->" << en;
        // prnv(all(curr));
    } while (next_permutation(all(curr)));
    print_google(to_string(mx));
}
