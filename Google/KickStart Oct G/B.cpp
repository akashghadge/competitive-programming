...
int circle(int x1, int y1, int x2, int y2, int r1, int r2)
{
    ...
    if (d <= abs(r1 - r2) or d == r1 + r2 or d < r1 + r2)
    {
        return 1;
    }
    else
...
double distance(int x2, int y2)
{
    ...
    return sqrt(pow(x2, 2) + pow(y2, 2) * 1.0);
}
void sol()
{
    var2(disk, house);
    ...
    for (auto val : red)
    {
        total.push_back({1, val});
    }
    for (auto val : yellow)
    {
        total.push_back({2, val});
    }
    sort(all(total), [](auto f, auto s)
         { return distance(f.second.first, f.second.second) < distance(s.second.first, s.second.second); });
    int red_cn = 0;
    int yellow_cn = 0;
    bool red_found = 0;
    bool yellow_found = 0;
    for (auto val : total)
    {
        // cout << val.first << " -> " << val.second.first << " " << val.second.second << en;
        if (circle(0, 0, val.second.first, val.second.second, house, disk))
        {
            if (val.first == 1 and !yellow_found)
            {
                red_found = 1;
                red_cn++;
            }
            if (val.first == 2 and !red_found)
            {
                yellow_found = 1;
                yellow_cn++;
            }
    ...
    print_google(to_string(red_cn) + " " + to_string(yellow_cn));
}
