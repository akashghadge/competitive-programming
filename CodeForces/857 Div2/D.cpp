...
void sol()
{
    ...
    vi sorted;
    for (auto val : arr)
        b.push_back({val.second, i++});
    sort(all(b));
    for (auto val : b)
        sorted.push_back(val.first);
    i = 0;
    ll mn = INT_MAX;
    for (auto val : arr)
    {
        ll curr = val.first;
        ll id = lower_bound(all(sorted), curr) - sorted.begin();
        ...
        if (id == n)
        {
            ...
            if (i != b[id].second)
            {
                mn = min(mn, abs(curr - sorted[id]));
            }
            if (id + 1 < n and b[id + 1].second != i)
            {
                mn = min(mn, abs(curr - sorted[id + 1]));
            }
            if (id - 1 >= 0 and b[id - 1].second != i)
            {
                mn = min(mn, abs(curr - sorted[id - 1]));
            }
        }
