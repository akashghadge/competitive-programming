...
/******************************** Main Section Start *********************************/
bool valid(vvi &a, int n, int m, int r, int c)
{
    if ((a[r][c] ^ a[r][c + 1] ^ a[r + 1][c] ^ a[r + 1][c + 1]) !=
        (a[r + 2][c + 2] ^ a[r + 2][c + 3] ^ a[r + 3][c + 2] ^ a[r + 3][c + 3]))
        return false;
    if ((a[r][c + 2] ^ a[r][c + 3] ^ a[r + 1][c + 2] ^ a[r + 1][c + 3]) !=
        (a[r + 2][c] ^ a[r + 2][c + 1] ^ a[r + 3][c] ^ a[r + 3][c + 1]))
        return false;
    return true;
}
bool check(vvi &arr, int n, int m)
{
    for (int i = 0; i <= (n - 4); i++)
    {
        for (int j = 0; j <= (m - 4); j++)
        {
            if (!valid(arr, n, m, i, j))
            {
                return false;
            }
        }
...
void sol()
{
    ...
    {
        ll ptr = 0;
        ll curr = i == 0 ? 2 : arr[i - 1][m - 1] + 1;
        for (int j = 0; j < m; j++)
        {
            if (ptr != 0 and ptr % 2 == 0)
            {
                curr += 2;
            }
            arr[i][j] = curr;
    ...
    check(arr, n, m) ? cout << "YES" : cout << "NO";
    cout << en;
    prn(n * 1LL * m);
    for (auto row : arr)
        prnv(all(row));
