...
void sol()
{
    ...
    // first n-1 rounds
    vi left = nge(arr, n);
    vi right = nge_right(arr, n);
    // prnv(left.begin(), left.end());
    // prnv(right.begin(), right.end());
    for (auto q : query)
    {
        if (left[q.first - 1] == -1 and right[q.first - 1] == -1)
        {
            int total = q.second;
            if (q.first == 1 or q.first == 2)
                cout << total << en;
            else
                cout << max(total - q.first + 2, 0) << en;
        }
        else if (left[q.first - 1] != -1)
        {
            cout << 0 << en;
        }
        else if (right[q.first - 1] != -1)
        {
            int total = q.second;
            if (q.second < q.first - 1)
                cout << 0 << en;
            else
                cout << n - 1 - right[q.first - 1] - q.first - 1 << en;
        }
        else
        {
            cout << 0 << en;
