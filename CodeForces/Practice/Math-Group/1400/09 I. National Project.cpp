...
void sol()
{
    ...
    if (n % 2 == 1)
        need++;
    ll iterations = need / g;
    if (need % g == 0)
    {
        ll res = (iterations * g) + ((iterations - 1) * b);
        ll can_be_bad = n - need;
        ll res2 = max(0LL, can_be_bad - ((iterations - 1) * b));
        prn(res + res2);
    }
    else
    {
        ll res = (iterations * g) + ((iterations)*b);
        res += (need % g);
        ll can_be_bad = n - need;
        ll res2 = max(0LL, can_be_bad - (iterations * b));
        prn(res + res2);
    }
}
