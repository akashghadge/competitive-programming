...
void dfs(vvi &arr, int n, int k, int r, int c)
{
...
bool valid(vvi &arr, int n, int k, int r, int c)
{
    // cout << " for " << r << " " << c << en;
    // for (auto val : arr)
    // {
    ...
    {
        if (0 <= i and i < n and arr[i][c])
            return false;
    }
    for (int j = c - k; j <= c + k; j++)
    {
        if ((0 <= j and j < n) and arr[r][j])
            return false;
    }
    return true;
}
