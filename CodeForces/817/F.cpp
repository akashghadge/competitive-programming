...
bool valid_mark(int i, int j, int n, int m, vvi &arr, vvi &visited)
{
    if (!arr[i][j] or visited[i][j])
        return false;
    if ((i + 1 >= n) or !arr[i + 1][j] or visited[i + 1][j])
        return false;
    if ((i + 1 >= n) or (j + 1 >= m) or !arr[i + 1][j + 1] or visited[i + 1][j + 1])
        return false;
    visited[i][j] = true;
    visited[i + 1][j] = true;
...
bool sol()
{
    ...
    if (st.size() == 4 and st[4] == st[1] and st[1] == st[2])
    {
        return true;
    }
    return false;
